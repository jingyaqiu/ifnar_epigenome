...
metadat <- data.frame(condition=rep(c("B16_WT", "B16_SKO", "R499_WT", "R499_SKO"), each=6), rep=rep(c("rep1", "rep2"), 12), histone=rep(rep(c("H3K4me1", "H3K4me3", "H3K27Ac"), each=2), 4))
metadat$sampleID <- paste(metadat$condition, metadat$histone, metadat$rep, sep="_")

peaks.use <- "pooled" # IDR, pooled
bam_type <- "all"
combined_peaks <- "WT" # WT, ALL, conditions (B16_WT, B16_SKO, etc.)
